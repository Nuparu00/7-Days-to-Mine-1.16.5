...
public class ContainerGrill extends Container {
    ...
    public TileEntityGrill grill;

    public ContainerGrill(int windowID, PlayerInventory invPlayer, TileEntityGrill grill, IIntArray intArray) {
        super(ModContainers.COOKING_GRILL.get(), windowID);
        if (ModContainers.COOKING_GRILL.get() == null)
            throw new IllegalStateException(
                    "Must initialise container type before constructing a ContainerGrill!");
        ...
        // server Containers
        if(grill != null) {
            addSlot(new SlotItemHandler(grill.getInventory(), TileEntityGrill.EnumSlots.INPUT_SLOT.ordinal(), 47, 27));
            addSlot(new SlotItemHandler(grill.getInventory(), TileEntityGrill.EnumSlots.INPUT_SLOT2.ordinal(), 65, 27));
    ...
    public static ContainerGrill createContainerServerSide(int windowID, PlayerInventory playerInventory, TileEntityGrill grill){
        return new ContainerGrill(windowID, playerInventory, grill, grill.dataAccess);
    }

    public static ContainerGrill createContainerClientSide(int windowID, PlayerInventory playerInventory,
                                                           net.minecraft.network.PacketBuffer extraData) {
        return new ContainerGrill(windowID, playerInventory, (TileEntityGrill) playerInventory.player.level.getBlockEntity(extraData.readBlockPos()), new IntArray(4));
    }
    ...
    public static class SlotOutput extends SlotItemHandler {
        ...
        public SlotOutput(PlayerEntity player, TileEntityGrill grill,IItemHandler itemHandler, int index, int xPosition, int yPosition) {
