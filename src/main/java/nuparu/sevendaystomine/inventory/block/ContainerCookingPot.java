...
public class ContainerCookingPot extends Container {
    ...
    public TileEntityCookingPot cookingPot;

    public ContainerCookingPot(int windowID, PlayerInventory invPlayer, TileEntityCookingPot cookingPot, IIntArray intArray) {
        super(ModContainers.COOKING_POT.get(), windowID);
        if (ModContainers.COOKING_POT.get() == null)
            throw new IllegalStateException(
                    "Must initialise container type before constructing a ContainerCookingPot!");
        ...
        // server Containers
        if(cookingPot != null) {
            addSlot(new SlotItemHandler(cookingPot.getInventory(), TileEntityCookingPot.EnumSlots.INPUT_SLOT.ordinal(), 47, 27));
            addSlot(new SlotItemHandler(cookingPot.getInventory(), TileEntityCookingPot.EnumSlots.INPUT_SLOT2.ordinal(), 65, 27));
    ...
    public static ContainerCookingPot createContainerServerSide(int windowID, PlayerInventory playerInventory, TileEntityCookingPot cookingPot){
        return new ContainerCookingPot(windowID, playerInventory, cookingPot, cookingPot.dataAccess);
    }

    public static ContainerCookingPot createContainerClientSide(int windowID, PlayerInventory playerInventory,
                                                                net.minecraft.network.PacketBuffer extraData) {
        return new ContainerCookingPot(windowID, playerInventory, (TileEntityCookingPot) playerInventory.player.level.getBlockEntity(extraData.readBlockPos()), new IntArray(4));
    }

    @Override
    public boolean stillValid(PlayerEntity player) {
        return this.cookingPot != null && this.cookingPot.canPlayerAccessInventory(player);
    }
    ...
    public static class SlotOutput extends SlotItemHandler {
        ...
        public SlotOutput(PlayerEntity player, TileEntityCookingPot cookingPot,IItemHandler itemHandler, int index, int xPosition, int yPosition) {
        ...
        protected void checkTakeAchievements(ItemStack p_75208_1_) {
            ...
            if (!this.player.level.isClientSide) {
                cookingPot.awardUsedRecipesAndPopExperience(this.player);
            }
