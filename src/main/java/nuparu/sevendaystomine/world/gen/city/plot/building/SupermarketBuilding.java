...
public class SupermarketBuilding extends Building{

    private ResourceLocation RIGHT  = new ResourceLocation(SevenDaysToMine.MODID, "supermarket_right");
    private ResourceLocation LEFT  = new ResourceLocation(SevenDaysToMine.MODID, "supermarket_left");

    public SupermarketBuilding(int weight) {
        this(weight, 0);
    }

    public SupermarketBuilding(int weight, int yOffset) {
    ...
    public void generate(ServerWorld world, BlockPos pos, Direction facing, boolean mirror, Random rand) {
        ...
        Template template = templatemanager.get(RIGHT);
        if (template == null) {
            return;
        }
        ...
        pos = pos.above(yOffset).relative(facing, -13);
        ...
        template = templatemanager.get(LEFT);
        if (template == null) {
            return;
        }
