...
public class GasStationBuilding extends Building{

    private ResourceLocation REST = new ResourceLocation(SevenDaysToMine.MODID, "gas_station_0");

    public GasStationBuilding(int weight) {
        this(weight, 0);
    }

    public GasStationBuilding(int weight, int yOffset) {
        super(new ResourceLocation(SevenDaysToMine.MODID, "gas_station_1"), weight, yOffset);
        canBeMirrored = false;
    ...
    public void generate(ServerWorld world, BlockPos pos, Direction facing, boolean mirror, Random rand) {
        ...
        pos = pos.above(yOffset).relative(facing,-27);
        ...
        pos = pos.relative(facing, -size.getX() + 27 + 4);
        generateTemplate(world, pos, mirror, facing, placementsettings, template, hasPedestal, rand);
