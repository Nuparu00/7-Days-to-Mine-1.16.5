...
/*
Based on PieceLimitedJigsawManager by TelepathicGrunt
https://github.com/TelepathicGrunt/SevenDaysToMine/blob/e1bfd9e79bade3d020fcdd443b387f96d209a519/src/main/java/com/telepathicgrunt/sevendaystomine/world/structures/pieces/PieceLimitedJigsawManager.java
 */
public class FastJigsawManager {
    public static void assembleJigsawStructure(
        ...
        if(startPool.size() == 0){
            SevenDaysToMine.LOGGER.warn("Repurposed Structures: Empty or nonexistent start pool: {}  Crash is imminent", startPool.getName());
        }
        ...
        for(Object piece : components){
            if(piece instanceof AbstractVillagePiece){
                ...
                if(poolElement instanceof SingleJigsawPiece){
                    ResourceLocation pieceID = ((SingleJigsawPieceAccessor) poolElement).sevendaystomine_getTemplate().left().orElse(null);
            ...
            while (candidatePieces.size() > 0) {
                ...
                if (pieceNeededToSpawn.isPresent()) {
                    for (int i = 0; i < candidatePieces.size(); i++) {
                        ...
                        if (candidatePiece instanceof SingleJigsawPiece && ((SingleJigsawPieceAccessor) candidatePiece).sevendaystomine_getTemplate().left().get().equals(pieceNeededToSpawn.get())) { // Condition 1
                ...
                if(candidatePiece instanceof SingleJigsawPiece){
                    pieceName = ((SingleJigsawPieceAccessor) candidatePiece).sevendaystomine_getTemplate().left().get();
