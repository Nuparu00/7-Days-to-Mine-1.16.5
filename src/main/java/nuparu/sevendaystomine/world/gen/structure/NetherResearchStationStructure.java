...
public class NetherResearchStationStructure extends Structure<NoFeatureConfig> {
    /**
     ...
     */
    private static final List<MobSpawnInfo.Spawners> STRUCTURE_MONSTERS = ImmutableList.of(new MobSpawnInfo.Spawners(ModEntities.TWISTED_ZOMBIE_RAW, 10, 0, 4)
    );
    private static final List<MobSpawnInfo.Spawners> STRUCTURE_CREATURES = ImmutableList.of(
    );

    public NetherResearchStationStructure(Codec<NoFeatureConfig> codec) {
        super(codec);
    }
    ...
    public IStartFactory<NoFeatureConfig> getStartFactory() {
        return NetherResearchStationStructure.Start::new;
    }
    ...
    protected boolean isFeatureChunk(ChunkGenerator chunkGenerator, BiomeProvider biomeSource, long seed, SharedSeedRandom chunkRandom, int chunkX, int chunkZ, Biome biome, ChunkPos chunkPos, NoFeatureConfig featureConfig) {
        if (Utils.isNearStructure(Structure.NETHER_BRIDGE, chunkGenerator, seed, chunkRandom, chunkX, chunkZ, 5)) return false;
        if (Utils.isNearStructure(Structure.BASTION_REMNANT, chunkGenerator, seed, chunkRandom, chunkX, chunkZ, 5)) return false;
        ...
        for(int i = 32; i < 118;i++){
            BlockState state = columnOfBlocks.getBlockState(centerOfChunk.above(i));
    ...
    public static class Start extends StructureStart<NoFeatureConfig> {
        ...
        public void generatePieces(DynamicRegistries dynamicRegistryManager, ChunkGenerator chunkGenerator, TemplateManager templateManagerIn, int chunkX, int chunkZ, Biome biomeIn, NoFeatureConfig config) {
            ...
            for(int i = 32; i < 118;i++){
                BlockState state = blockReader.getBlockState(centerPos.above(i));
                if(state.getBlock() == Blocks.AIR && state.getFluidState().isEmpty()){
            ...
            JigsawManager.addPieces(
                    ...
                    new VillageConfig(() -> dynamicRegistryManager.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY)
                            ...
                            .get(new ResourceLocation(SevenDaysToMine.MODID, "scattered/nether/research_station_pool")),
