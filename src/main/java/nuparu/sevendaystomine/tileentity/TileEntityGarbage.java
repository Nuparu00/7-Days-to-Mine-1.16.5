...
public class TileEntityGarbage extends TileEntityItemHandler<ItemHandlerNameable>{
	...
	private static final ITextComponent DEFAULT_NAME = new TranslationTextComponent("container.garbage");
	protected int openCount;

	public TileEntityGarbage() {
		super(ModTileEntities.GARBAGE.get());
	}
	...
	public void onContainerOpened(PlayerEntity player) {
		if (this.openCount == 0) {
			this.level.playSound((PlayerEntity)null, worldPosition.getX()+0.5, worldPosition.getY()+0.5, worldPosition.getZ()+0.5, ModSounds.GARBAGE_OPEN.get(), SoundCategory.BLOCKS, MathUtils.getFloatInRange(0.45f,0.55f), MathUtils.getFloatInRange(0.75f,1.15f));
		}
		this.openCount++;
	}
