 ...
 *
 * This class is used to encapsulate the contents of the one of the zones of the
 * furnace (eg input zone, output zone, and fuel zone) and provide the link
 * between the parent TileEntity and the container. 1) stores information about
 * the items in the furnace: allows the container to manipulate the data stored
 * in the tile entity 2) provides a way for the container to ask the TileEntity
 * if certain actions are permitted (eg isUsableByPlayer, isItemValidForSlot) 3)
 * provides a way for the container to notify the TileEntity that the container
 * has changed (eg setChanged, openInventory)
 *
 * Typical usage for a TileEntity which needs to store Items: 1) When
 * constructing the TileEntity, create and store a FurnaceZoneContents using
 * createForTileEntity() 2) In your ContainerType<MyContainer>, create a
 * FurnaceZoneContents using createForClientSideContainer() and pass it to the
...
public class ItemZoneContents implements IInventory {
	...
	/**
	 * Use this constructor to create a FurnaceZoneContents which is linked to its
	 * parent TileEntity. On the server, this link will be used by the Container to
	 * request information and provide notifications to the parent On the client,
	 * the link will be unused. There are additional notificationLambdas available;
	 * these two are explicitly specified because your TileEntity will nearly always
	 * need to implement at least these two
	 * 
	 ...
	 */
	public static ItemZoneContents createForTileEntity(int size,
			Predicate<PlayerEntity> canPlayerAccessInventoryLambda, Notify markDirtyNotificationLambda) {
		return new ItemZoneContents(size, canPlayerAccessInventoryLambda, markDirtyNotificationLambda);
	}
	 ...
	 * @return the new ChestContents
	 */
	public static ItemZoneContents createForClientSideContainer(int size) {
		return new ItemZoneContents(size);
	}
	 ...
	 * @param itemStackToInsert the itemStack to insert. Is not mutated by the
	 *                          function.
	 * @return if successful insertion: ItemStack.EMPTY. Otherwise, the leftover
	 *         itemstack (eg if ItemStack has a size of 23, and only 12 will fit,
	...
	public ItemZoneContents(int size) {
		this.furnaceComponentContents = new ItemStackHandler(size);
	}

	private ItemZoneContents(int size, Predicate<PlayerEntity> canPlayerAccessInventoryLambda,
