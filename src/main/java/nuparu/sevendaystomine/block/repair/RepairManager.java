...
public class RepairManager {
	...
	public Repair getRepair(Block block, PlayerInventory inv) {
		Repair lastRepair = null;
		for (Repair repair : repairList) {
			if (repair.block == block) {
				lastRepair = repair;
				for (int slot = 0; slot < inv.getContainerSize(); slot++) {
					if (repair.getItemStack().getItem() == inv.getItem(slot).getItem()) {
						return repair;
					}
				}
			}
	...
	public static void handleMap(Map<String, Map<String, String>> map) {
		if (map.containsKey("add")) {
			...
			while (add.hasNext()) {
				...
				Block block =  ForgeRegistries.BLOCKS.getValue(new ResourceLocation(pair.getKey()));
				Item item = ForgeRegistries.ITEMS.getValue(new ResourceLocation(pair.getValue()));
	...
	public static Map<String, Map<String, String>> readRepairsFile(boolean local)
		...
		if (local) {
			fis = Utils.getInsideFileStream("data",SevenDaysToMine.MODID.toLowerCase() + "/data/repair.json");
		} else {
