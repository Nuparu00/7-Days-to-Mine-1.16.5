...
public class TileEntityTurretAdvancedRenderer extends TileEntityRenderer<TileEntityTurretAdvanced> {
    ...
    private final ResourceLocation TEX = new ResourceLocation(SevenDaysToMine.MODID, "entity/turret_advanced");
    ...
    public TileEntityTurretAdvancedRenderer(TileEntityRendererDispatcher p_i226006_1_) {
        super(p_i226006_1_);
        ...
        CameraBase = new ModelRenderer(64, 64, 14, 0);
        CameraBase.addBox(-4F, -7.5F, -3F, 2, 2, 4);
        CameraBase.setPos(0F, 9F, 0F);
        CameraBase.mirror = true;
        setRotation(CameraBase, 0F, 0F, 0F);
        CameraHolder = new ModelRenderer(64, 64, 19, 10);
        CameraHolder.addBox(-2F, -7F, -2F, 2, 1, 2);
        CameraHolder.setPos(0F, 9F, 0F);
    ...
    @Override
    public void render(TileEntityTurretAdvanced te, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {

        World world = te.getLevel();
        boolean flag = world != null;
        BlockState blockstate = flag ? te.getBlockState() : ModBlocks.TURRET_ADVANCED.get().defaultBlockState();
        Direction direction = blockstate.hasProperty(BlockTurretAdvanced.FACING) ? blockstate.getValue(BlockTurretAdvanced.FACING) : Direction.SOUTH;
        Block block = blockstate.getBlock();

        if (block instanceof BlockTurretAdvanced) {
            ...
            CameraBase.render(matrixStack,builder,combinedLight,combinedOverlay);
            CameraHolder.render(matrixStack,builder,combinedLight,combinedOverlay);
            matrixStack.popPose();
        }
    ...
    public void rotateHead(float rot) {
        ...
        this.CameraBase.yRot = this.HeadBase.yRot;
        this.CameraHolder.yRot = this.HeadBase.yRot;
    }
}
