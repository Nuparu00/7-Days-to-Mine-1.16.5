package nuparu.sevendaystomine.integration.jei.pot;
...
public class PotRecipeCategory implements IRecipeCategory<CookingPotRecipeShapeless> {
    ...
    public static final ResourceLocation ID = new ResourceLocation(SevenDaysToMine.MODID, "cooking_pot");

    public PotRecipeCategory(IGuiHelper helper) {
        background = helper.createDrawable(TEXTURE, 4, 4, 156, 79);
        icon = helper.createDrawableIngredient(new ItemStack(ModBlocks.COOKING_POT.get()));
        name = "Cooking Pot";
    ...
    protected IDrawableAnimated getArrow(CookingPotRecipeShapeless recipe) {
    ...
    protected void drawExperience(CookingPotRecipeShapeless recipe, MatrixStack matrixStack, int y) {
    ...
    protected void drawCookTime(CookingPotRecipeShapeless recipe, MatrixStack matrixStack, int y) {
        int cookTime = recipe.getCookingTime();
        if (cookTime > 0) {
    ...
    @Override
    public Class<? extends CookingPotRecipeShapeless> getRecipeClass() {
        return CookingPotRecipeShapeless.class;
    }
    ...
    @Override
    public void setIngredients(CookingPotRecipeShapeless recipe, IIngredients ingredients) {
        ingredients.setInputIngredients(recipe.getIngredients());
        ingredients.setOutput(VanillaTypes.ITEM,recipe.getResultItem());
    }

    @Override
    public void setRecipe(IRecipeLayout recipeLayout, CookingPotRecipeShapeless recipe, IIngredients ingredients) {
    ...
    @Override
    public void draw(CookingPotRecipeShapeless recipe, MatrixStack matrixStack, double mouseX, double mouseY) {
