...
public class RocketEntity extends ProjectileEntity {
    ...
    private static final DataParameter<Byte> PIERCE_LEVEL = EntityDataManager.defineId(RocketEntity.class, DataSerializers.BYTE);
    ...
    public RocketEntity(EntityType<RocketEntity> type, World p_i48546_2_) {
        super(type, p_i48546_2_);
    }

    public RocketEntity(World p_i48546_2_) {
        super(ModEntities.SHOT.get(), p_i48546_2_);
    }

    public RocketEntity(double p_i48547_2_, double p_i48547_4_, double p_i48547_6_, World p_i48547_8_) {
        this(ModEntities.ROCKET.get(), p_i48547_8_);
        this.setPos(p_i48547_2_, p_i48547_4_, p_i48547_6_);
    }

    public RocketEntity(LivingEntity p_i48548_2_, World p_i48548_3_) {
    ...
    public void tick() {
        ...
        if (this.isInWater()) {
            for (int i = 0; i < 4; ++i) {
                float f3 = 0.25F;
                this.level.addParticle(ParticleTypes.BUBBLE, this.getX() - motion.x * 0.25D,
            ...
            for (int i = 0; i < 10; ++i) {
                this.level.addParticle(ParticleTypes.LARGE_SMOKE,
                        this.getX() - motion.x * 0.25D + MathUtils.getDoubleInRange(0, 0.8),
                        this.getY() - motion.y * 0.25D + MathUtils.getDoubleInRange(0, 0.8),
                        this.getZ() - motion.z * 0.25D + MathUtils.getDoubleInRange(0, 0.8), -motion.x / 10,
                        -motion.y / 10, -motion.z / 10);
    ...
    protected void onHitEntity(EntityRayTraceResult p_213868_1_) {
        ...
        if (!level.isClientSide()) {
            level.explode(this, entity.getX(), entity.getY(), entity.getZ(), 3f, sparking, Explosion.Mode.DESTROY);
        }
    ...
    protected void onHitBlock(BlockRayTraceResult rayTraceResult) {
        ...
        if (!level.isClientSide()) {
            level.explode(this, pos.getX(), pos.getY(), pos.getZ(), 3f, sparking, Explosion.Mode.DESTROY);
        }
    }
